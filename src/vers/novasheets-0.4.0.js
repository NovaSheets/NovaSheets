...
function parseNovaSheets() {
    ...
    for (let s in stylesheetContents) {
        ...
        let cssOutput = '';
        for (let i in lines) {
            cssOutput += lines[i].replace(/^(.*?)\s?\/\/.+$/, '$1').replace(/[\r\n]/g, ''); // remove CRLF and comments
        }
        ...
        while ((cssOutput.indexOf('$(') > -1 || loop < 2) && loop++ < MAX_RECURSION) {
            for (let i in customVars) {
                ...
                let varPartsRegex = "\\s*(?:\\|\\s*([^|$()]+)\\s*)?"; //= '|param=arg'
            ...
            cssOutput = cssOutput
                ...
                .replace(nssFunction('@radians', number, 1), args => Number(getArgs(args, 1)) * Math.PI / 180)
                .replace(nssFunction('@gradians', number, 1), args => Number(getArgs(args, 1)) * Math.PI / 200)
