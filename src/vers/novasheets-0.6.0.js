#!/usr/bin/env node
// NovaSheets 0.6.0 //
const NOVASHEETS_VERSION = "0.6.0";
...
function parseNovaSheets(rawInput) {
    ...
    for (let s in stylesheetContents) {
        ...
        while ((cssOutput.indexOf('$(') > -1 || loop < 1) && loop++ < MAX_RECURSION) {
            ...
            const parseHex = val => {
                let a = val.replace('#', '');
                if (!a) return rgbFromHex('000000', '00');
                else if (a.length === 1) return rgbFromHex(a.repeat(6));
                else if (a.length === 2) return rgbFromHex(a[0].repeat(6), a[1].repeat(2));
                else if (a.length === 3) return rgbFromHex(a[0] + a[0] + a[1] + a[1] + a[2] + a[2]);
                else if (a.length === 4) return rgbFromHex(a[0] + a[0] + a[1] + a[1] + a[2] + a[2], a[3] + a[3]);
                else if (a.length === 6) return rgbFromHex(a.split(/(?=..)/).join(''));
                else return rgbFromHex(a.substr(0, 6).padEnd(6, '0'), a.substr(6, 2) || null);
            }
            ...
            nssFunction('@colou?r', (_, type, a = '0', b = '0', c = '0', d = '') => {
                ...
                if (/#|hash|hex/i.test(type)) {
                    return '#' + toHex(a) + toHex(b) + toHex(c) + (d ? toHex(fromPercent(d)) : '');
                }
                else if (/rgba?/i.test(type)) {
                    if (type === 'rgba') return `rgba(${a}, ${b}, ${c}, ${d || d === 0 ? 100 : ''}%)`;
                    return `rgb(${a}, ${b}, ${c})`;
                }
                else if (/hsla?/i.test(type)) {
                    if (type === 'hsla') return `${type}(${a % 360}, ${b}%, ${c}%, ${d || d === 0 ? 100 : ''}%)`;
            ...
            const blendColors = (color1, color2, amt) => {
                ...
                if (type === 'rgba') return `rgba(${r}, ${g}, ${b}, ${a})`;
                else if (type === 'rgb') return `rgb(${r}, ${g}, ${b})`;
                else if (type === 'hsla') return `hsla(${r % 360}, ${g / 100}%, ${b / 100}%, ${a})`;
                else if (type === 'hsl') return `hsla(${r % 360}, ${g / 100}%, ${b / 100}%)`;
                else if (type === '#') return hexFromRgb([r, g, b, a]);
                else return `${type}(${r}, ${g}, ${b})`;
            }
            ...
            nssFunction('@contrast', (_, color, lightVal, darkVal) => (parseLuma(color) < 0.5/*'is dark?':*/) ? lightVal : darkVal);
            nssFunction('@gr[ae]yscale', (_, color) => {
                if (color.startsWith('hsl')) return color.replace(/^hsla?\s*\(\s*(\d+),\s*(\d+)/, 'hsla($1, 0')
            ...
            /// CSS functions
            nssFunction('@breakpoint', (_, a = '', b, c, d = '') => {
                return `@media (max-width: ${a}+0px) {\n ${b} {${c || ''}}\n}\n@media (min-width: ${a || ''}+1px) {\n${b} {${d}}\n}`;
            }, { notrim: true, nonest: true });
