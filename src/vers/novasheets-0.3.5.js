    ...
    for (let s in stylesheetContents) {
        ...
        while ((cssOutput.indexOf('$(') > -1 || loop < 2) && loop++ < MAX_RECURSION) {
            for (let i in customVars) {
                ...
                cssOutput = cssOutput.replace(RegExp(replaceRegex), styles[customVars[i].name]); // substitute 'name'
                ...
                for (let j = 0; j < varArgs.length; j++) {
                    ...
                    for (let k in localVars) {
                        ...
                        cssOutput = cssOutput.replace(RegExp(localvarFormatted, 'g'), val); // substitute 'val'
                    }
                }
        ...
        // Cleanup output
        cssOutput = cssOutput.replace(/;\s*;/g, ';').replace(/\s+/g, ' ').replace(/} ?/g, '}\n');
